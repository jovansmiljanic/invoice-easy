...
interface File {
  invoices: Invoice[]; // Changed to an array of invoices
  myAccount?: MyAccount | null;
  content?: IContentValues;
}

const index: FC<File> = ({ myAccount, invoices, content }) => {
  const totalPrice = (items: Invoice["items"], tax: Invoice["tax"]) => {
    ...
    return total + (total * Number(tax)) / 100;
  };

  const subTotalPrice = (items: Invoice["items"]) => {
    const total = items.reduce((acc, item) => {
      const price = Number(item.price) * Number(item.qty);
  ...
  return (
    <Document>
      {invoices?.map((invoice, index) => (
        <Page key={index} size="A4" style={styles.newInvoice}>
          {myAccount?.logo ? (
            <>
              ...
              <View style={styles.details}>
                <View style={styles.col}>
                  ...
                  <Text>
                    {myAccount?.zipCode}, {myAccount?.city},{" "}
                    {myAccount?.country}
                  </Text>
                  <Text>
          ...
          <View style={styles.total}>
            <Text>
              {" "}
              {content?.subTotal}: {subTotalPrice(invoice.items)}{" "}
              {content?.currency}
            </Text>
            ...
            <Text>
              {content?.total}: {totalPrice(invoice.items, invoice.tax)}{" "}
              {content?.currency}
            </Text>
          </View>
