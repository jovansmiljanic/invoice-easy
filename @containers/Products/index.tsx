...
const index: FC = ({}) => {
  ...
  const [updatedItems, setupdatedItems] = useState<Product[]>([]);
  ...
  const handleChangeType = (selected: any) => {
    push(`/products/?&page=${0}&limit=${limit ? limit : selected.value}`);
  };
  ...
  const fetchItems = async ({ page }: IFetch) => {
    ...
    const clientUrl = `/api/products/?${queryUrl}${searchUrl}&limit=${limit}&skip=${pageMemo}`;
            ...
            <Wrapper>
              <OptionsWrap>
                ...
                <Button
                  ...
                  size="small"
                  onClick={() => {
                    setIsProductModalOpen(!isProductModalOpen), setClientData();
                  }}
                  margin={{
              ...
              <SearchWrap>
                <Search
                  placeholder={t("product:searchLabel")}
                  searchQuery={searchQuery}
